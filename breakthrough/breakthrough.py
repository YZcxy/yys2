...
class Breakthrough(Fighter):
    ...
    def fight_and_quit(self, target_pos, mood):
        ...
        self.click_until('突破目标', 'img\\JIN-GONG.png', *self.valid_position(target_pos),
                         step_time=mood.get1mood() / 1000, appear=True)
        ...
        self.click_until('确认退出', 'img\\QUE-REN-TUI-CHU.png', *BreakthroughPos.confirm_quit_btn,
                         step_time=mood.get1mood() / 1000, appear=False)
    ...
    def fighting(self, target_pos, mood):
        ...
        if self.already_break(target_pos):
            self.log.warning("目标已被突破过了，跳过")
            return 1

        time.sleep(1)

        # 点击有效位置，直到可以进攻按钮
        self.click_until('突破目标', 'img\\JIN-GONG.png', *self.valid_position(target_pos), step_time= mood.get1mood()/1000, appear=True)

        time.sleep(1)

        # 点击进攻，开始突破
        self.click_until('进攻', 'img\\JIN-GONG.png', *self.attack_position(target_pos), step_time= mood.get1mood()/1000, appear=False)
        ...
        while self.run:
            ...
            for i in range(9):
                if not self.run:
                    break
                if victories >= 3:
                    self.log.info('成功突破:'+str(victories)+'次，等待新的一轮')
                    break
                result = self.fighting(BreakthroughPos.target_position[i], mood)
                if not result:
                    self.log.warning("强制退出脚本")
                    sys.exit(0)
                if result == 1:
                    victories += 1
                    self.max_tasks -= 1
                self.log.warning("当前挑战次数->" + str(i + 1) + "，当前成功突破次数->" + str(victories))
